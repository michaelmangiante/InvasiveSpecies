...
# load data
setwd("K:/InvasiveSpecies/FinalData/animalsDB/CircularFlow")
#inData <- read.csv(file="animals_USSpecies_CircularFlowData.csv", header=TRUE, sep=",")
#metadata <- read.csv(file="Circle_metadata3.csv", header = TRUE, sep=",")

inData <- read.csv(file="animals_USSpecies_CircularFlowData.csv", stringsAsFactors = FALSE)
metadata <- read.csv(file="Circle_metadata3.csv", stringsAsFactors=FALSE)
...
#print jpeg
jpeg("Animals_CircularPlot_FirstUSOccurance.jpeg", width=11,heigh=11, units="in", res = 600)
...
circos.trackPlotRegion(
  ...
  panel.fun = function(x, y) {
    ...
    circos.axis(h = "top", 
                major.at = seq(from = 0, to = xlim[2], by = ifelse(test = xlim[2]>10, yes = 10, no = 2)), 
