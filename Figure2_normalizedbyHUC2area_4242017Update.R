...
indf <- inData[,c("HUC2_Value", "Year", "Cumulative_Occurances_P_HUC2normalized", "Cumulative_Occurances_A_HUC2normalized", "Cumulative_First_Occurance_P_HUC2normalized", "Cumulative_First_Occurance_A_HUC2normalized", "CumulativeHUC12Area_ProportionofTotal_P", "CumulativeHUC12Area_ProportionofTotal_A")]
...
head(meltdf)
levels(meltdf$variable) <- c("Cumulative Annual Plant Records", "Cumulative Annual Animal Records", "Cumulative First Plant \nSpecies Observations","Cumulative First Animal \nSpecies Observations", "Proportional Accumulation of HUC12 \nSq. Km. with Plant Observations",   "Proportional Accumulation of HUC12 \nSq. Km. with Animal Observations")


# Plot

jpeg("Figure2_normalized_TwoColumn_blankLabel.jpeg", width=8.5,heigh=11, units="in", res = 600)
...
theme_bw()+
theme(legend.position = "bottom", panel.border =element_rect(colour = "black", fill=NA), panel.grid.major = element_blank(), 
panel.grid.minor = element_blank(), axis.line= element_line(colour = "black"), strip.text = element_blank()) 
# "right" or "bottom
